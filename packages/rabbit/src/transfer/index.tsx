...
export default function Transfer({
  ...
  const handleSelectedChange = useCallback(
    (index: number, changeOptions: TransferOptionType[], selected?: boolean) => {
  ...
  const handleTransfer = useCallback(
    (index: number, dir: TransferDirection) => {
        ...
        <div
          ...
          {index != _data.length - 1 && (
            <div className="btn-wrapper">
              <span
                onClick={() => handleTransfer(index, 'right')}
                className={classNames('btn', !_selectedValues[index]?.length && 'is-disabled')}>
                {btnRender ? btnRender(index, 'right') : <Icon type="arrowRight" />}
              </span>
              {!(isArray<boolean[]>(oneWay) ? oneWay[index] : oneWay) && (
                <span
                  onClick={() => handleTransfer(index + 1, 'left')}
                  className={classNames('btn', !_selectedValues[index + 1].length && 'is-disabled')}>
