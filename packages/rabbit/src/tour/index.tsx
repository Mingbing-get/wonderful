...
export default function Tour({ steps, open = false, arrow, placement, current = 0, finishButtonProps, tip = 'dot', onClose, onFinish, onChange }: TourProps) {
    ...
    return (
      <div className="rabbit-tour-popover">
        <div className="rabbit-tour-header">
          <div>{steps[_current].title}</div>
          <Icon
            type="close"
            onClick={handleClose}
          />
        </div>
        <div className="rabbit-tour-content">{steps[_current].content}</div>
          ...
          {tip === 'dot' && (
            <ul className="tour-dots">
              {steps.map((_, index) => (
                <li
                  key={index}
                  className={classNames(index === _current && 'is-active')}
                />
              ))}
            </ul>
          )}
  ...
  return createPortal(
    <div className="rabbit-tour-wrapper rabbit-component">
