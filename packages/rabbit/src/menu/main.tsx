...
import { MenuItem, MenuSubMenuType, MenuMode, MenuTheme } from './index'
...
export default function MenuMain({
  ...
  return (
    <div
      className={classNames('rabbit-menu-wrapper', `menu-theme-${theme}`, `menu-mode-${mode}`, level !== 0 && 'no-border-right', className)}
      style={style}
    >
      {
        items.map(item => (
          <div
            className={classNames('menu-item', item.disabled && 'is-disabled', openPath.includes(item.key) && 'is-open', selectPath.includes(item.key) && 'is-select')}
            key={item.key}
            onClick={e => {
          ...
          >
            <div className='menu-item-title' style={mode === 'inline' ? { paddingLeft: `${inlineLevel +0.5}rem` } : undefined}>
              ...
              {mode === 'vertical' && !item.disabled && hasChildren(item) && (
                <Icon type='arrowRight' className='menu-arrow' />
              )}
            </div>
            {
